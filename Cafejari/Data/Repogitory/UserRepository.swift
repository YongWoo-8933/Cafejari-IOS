...
protocol UserRepository {
    ...
    func fetchMyWeekRanking(accessToken: String) async throws -> [LeaderResponse]
    func fetchMyMonthRanking(accessToken: String) async throws -> [LeaderResponse]
}
...
final class UserRepositoryImpl: UserRepository {
    ...
    func fetchMyMonthRanking(accessToken: String) async throws -> [LeaderResponse] {
        do {
            let urlSession = URLSession.shared
            let request = customUrlRequest.get(urlString: httpRoute.myMonthRanking(), accessToken: accessToken)
            let (data, urlRes) = try await urlSession.data(for: request)
    ...
    func fetchMyWeekRanking(accessToken: String) async throws -> [LeaderResponse] {
        do {
            let urlSession = URLSession.shared
            let request = customUrlRequest.get(urlString: httpRoute.myWeekRanking(), accessToken: accessToken)
            let (data, urlRes) = try await urlSession.data(for: request)
